...
export const MenuSheet = ({ onOpenSettings, onOpenExtensions }: MenuSheetProps) => {
  ...
  const renderMenuTile = (id: MenuTileId) => {
    ...
    if (id === 'extensions') {
      return (
        <Pressable key={id} {...baseProps}>
          <MaterialIcons name="extension" size={18} color={theme.text} style={styles.actionIcon} />
          <Text style={[styles.actionText, { color: theme.text }]}>{getMenuTileLabel(id)}</Text>
        </Pressable>
      );
    }
